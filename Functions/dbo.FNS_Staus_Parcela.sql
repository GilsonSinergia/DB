...
SET ANSI_NULLS ON
GO
CREATE   Function [dbo].[FNS_Staus_Parcela] ( @date Date)
Returns Table
AS
Return
  (
...
SELECT 
  ...
  MovFina0.Valor,
  MovFina0.Valor - CONVERT(MONEY, ISNULL(ValorLiquido,0))  VL_Aberto,  
  ISNULL(ValorLiquido,0)VL_Quitado,
  ISNULL(ValorEncargos,0)VL_Encargo,
  ISNULL(ValorDesconto,0)VL_Desconto,
  ISNULL(ValorPago,0)VL_Pago
FROM MovFina0
